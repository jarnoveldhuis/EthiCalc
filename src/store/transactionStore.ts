...
import {
  Transaction,
  // AnalyzedTransactionData, // REMOVED - Unused
  Charity // Ensure Charity type is imported if needed by ApiAnalysisResultItem
...
// import { processTransactionList } from "@/features/analysis/transactionAnalysisService"; // REMOVED - Unused
...
// Interface for items returned by /api/analysis
...
export interface TransactionState {
  ...
  // totalSocietalDebt: number; // Likely superseded
...
export const useTransactionStore = create<TransactionState>((set, get) => ({
  ...
  connectBank: async (publicToken, user) => {
      if (!user || isAnyLoading(get())) { console.log("connectBank: Skipping."); return; }
      set({ isConnectingBank: true, connectionStatus: { isConnected: false, error: null } });
      try {
          const apiUrl = "/api/banking/exchange_token";
          const requestOptions: RequestInit = { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ public_token: publicToken })};
          ...
          set((state) => ({ connectionStatus: { ...state.connectionStatus, isConnected: true } }));
          try { sessionStorage.removeItem('wasManuallyDisconnected'); } catch (e) { console.error(e); }
          await get().fetchTransactions(data.access_token);
      } catch (error) {
          console.error("Error connecting bank:", error);
          set({ connectionStatus: { isConnected: false, error: error instanceof Error ? error.message : "Failed to connect bank" } });
      } finally {
  ...
  fetchTransactions: async (accessToken) => {
      ...
      try {
          if (!tokenToUse) { const storedData = localStorage.getItem("plaid_access_token_info"); if (!storedData) throw new Error("No access token available"); tokenToUse = JSON.parse(storedData).token; }
          ...
          if (!response.ok) { const errorData = await response.json().catch(() => ({})); if (response.status === 400 && errorData.details?.includes("ITEM_LOGIN_REQUIRED")) { get().resetState(); throw new Error("Bank connection expired. Please reconnect."); } throw new Error(`Plaid fetch failed: ${response.status} ${errorData.details || "Unknown Plaid error"}`); }
          ...
          set((state) => ({ isFetchingTransactions: false, connectionStatus: { ...state.connectionStatus, isConnected: true, error: mappedTransactions.length === 0 ? "No transactions found" : null }, }));
          if (mappedTransactions.length > 0) { await get().analyzeAndCacheTransactions(mappedTransactions); }
          else { if (get().isAnalyzing) set({ isAnalyzing: false }); get().analyzeAndCacheTransactions([]); }
      } catch (error) {
  ...
  analyzeAndCacheTransactions: async (incomingTransactions) => {
      if (get().isAnalyzing) { console.log("analyzeAndCacheTransactions: Skipping, already analyzing."); return; }
      ...
      const cacheLookupPromises = transactionsToProcess
          ...
          .map(async (tx) => { /* ... cache lookup logic ... */
               const vendorName = tx.name; const normalizedName = normalizeVendorName(vendorName);
               if (normalizedName !== "unknown_vendor") { try { const cachedData = await getVendorAnalysis(normalizedName); return { tx, cachedData }; } catch (error) { console.error(`Error fetching cache for ${normalizedName}:`, error); return { tx, cachedData: null }; } }
      ...
      cacheResults.forEach(({ tx, cachedData }) => { /* ... process cache results ... */
          const txId = getTransactionIdentifier(tx); if (!txId) return;
          if (cachedData) { const updatedTx: Transaction = { /* merge */ ...tx, unethicalPractices: cachedData.unethicalPractices || [], ethicalPractices: cachedData.ethicalPractices || [], practiceWeights: cachedData.practiceWeights || {}, practiceSearchTerms: cachedData.practiceSearchTerms || {}, practiceCategories: cachedData.practiceCategories || {}, information: cachedData.information || {}, citations: cachedData.citations || {}, analyzed: true, }; transactionsFromCache.push(updatedTx); transactionsById.set(txId, updatedTx); }
          else { transactionsForApi.push(tx); }
       });
      transactionsToProcess.forEach(tx => { /* ... populate alreadyAnalyzed list ... */
           const txId = getTransactionIdentifier(tx);
           if (tx.analyzed && txId && !transactionsById.get(txId)?.analyzed ) { alreadyAnalyzed.push(tx); }
           else if (tx.analyzed && txId && transactionsById.get(txId)?.analyzed){ if(!alreadyAnalyzed.some(analyzedTx => getTransactionIdentifier(analyzedTx) === txId)) { alreadyAnalyzed.push(transactionsById.get(txId)!); } }
       });
      ...
      try {
          if (transactionsForApi.length > 0) {
              ...
              analysisResponse.transactions.forEach((analyzedTx: ApiAnalysisResultItem) => { /* populate map */ if (analyzedTx.plaidTransactionId) { openAiResultsMap.set(analyzedTx.plaidTransactionId, analyzedTx); } });
              set({ isSavingCache: true });
              const cacheSavePromises: Promise<void>[] = [];
              apiAnalyzedResults = transactionsForApi.map(originalTx => { /* ... merge API results and prepare cache saves ... */
                  const originalId = originalTx.plaidTransactionId;
                  if (originalId && openAiResultsMap.has(originalId)) {
                      const analyzedVersion: ApiAnalysisResultItem = openAiResultsMap.get(originalId)!;
                      const finalTxData: Transaction = { /* merge */ ...originalTx, societalDebt: analyzedVersion.societalDebt, unethicalPractices: analyzedVersion.unethicalPractices || [], ethicalPractices: analyzedVersion.ethicalPractices || [], practiceWeights: analyzedVersion.practiceWeights || {}, practiceDebts: analyzedVersion.practiceDebts || {}, practiceSearchTerms: analyzedVersion.practiceSearchTerms || {}, practiceCategories: analyzedVersion.practiceCategories || {}, charities: analyzedVersion.charities || {}, information: analyzedVersion.information || {}, citations: analyzedVersion.citations || {}, analyzed: true, };
                      const vendorNameForCache = originalTx.name; const normalizedNameForCache = normalizeVendorName(vendorNameForCache);
                      if (normalizedNameForCache !== "unknown_vendor") {
                          const vendorDataToSave: Omit<VendorAnalysis, 'analyzedAt'> = { /* data */ originalName: vendorNameForCache, analysisSource: 'openai', unethicalPractices: finalTxData.unethicalPractices, ethicalPractices: finalTxData.ethicalPractices, practiceWeights: finalTxData.practiceWeights, practiceSearchTerms: finalTxData.practiceSearchTerms, practiceCategories: finalTxData.practiceCategories, information: finalTxData.information, citations: finalTxData.citations, };
          ...
          set({ transactions: finalTransactions, impactAnalysis: finalImpact, creditState: { ...currentCreditState, availableCredit: finalImpact.availableCredit }, isAnalyzing: false, connectionStatus: { ...get().connectionStatus, error: null }, });
  ...
  applyCredit: async (amount, userId) => {
        ...
        try {
            ...
            set({ creditState: { ...creditState, ...updatedCreditStateValues, availableCredit: newAnalysis.availableCredit }, impactAnalysis: newAnalysis });
            return true;
  ...
  loadLatestTransactions: async (userId): Promise<boolean> => {
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                const loadedTransactions = ((docData.transactions as Transaction[]) || []).map((tx) => ({ ...tx, analyzed: true })); // Assume saved are analyzed
                ...
                set({ transactions: loadedTransactions, savedTransactions: loadedTransactions, impactAnalysis: analysis, hasSavedData: true, creditState: { ...get().creditState, availableCredit: analysis.availableCredit, appliedCredit: initialAppliedCredit }, connectionStatus: { isConnected: true, error: null }, });
                success = true;
  ...
  initializeStore: async (user: User | null) => {
        ...
        try {
            ...
            console.log(`initializeStore: loadLatestTransactions result: ${loadedFromFirebase}`);
            ...
            if (!loadedFromFirebase && hasValidStoredToken && tokenToFetch) { console.log("initializeStore: No Firebase data, valid token exists. Fetching fresh..."); await get().fetchTransactions(tokenToFetch); }
            else if (!loadedFromFirebase && !hasValidStoredToken) { console.log("initializeStore: No Firebase data and no valid token."); set((state) => ({ connectionStatus: { ...state.connectionStatus, isConnected: false, error: null } })); }
            else if (loadedFromFirebase) { console.log("initializeStore: Loaded from Firebase. Ensure connected status."); set((state) => ({ connectionStatus: { ...state.connectionStatus, isConnected: true, error: null } })); }
        ...
        finally { set({ isInitializing: false }); console.log("initializeStore: Finished."); }
    },
