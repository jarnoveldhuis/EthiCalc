...
const AnalysisResultTransactionSchema = z.object({
  ...
  citations: z.record(z.array(CitationObjectSchema))
                 .optional().default({})
                 .catch({}),
}).passthrough();
...
function isPotentialCitation(obj: unknown): obj is { url: string; title?: unknown } {
...
export async function analyzeTransactionsViaAPI(
  transactionsToAnalyze: Transaction[]
   ...
   const sanitizedTransactions = transactionsToAnalyze.map((tx) => ({
         plaidTransactionId: tx.plaidTransactionId, date: tx.date || "N/A", name: tx.name || "Unknown Merchant",
         amount: typeof tx.amount === "number" ? tx.amount : 0, plaidCategories: tx.plaidCategories || [], location: tx.location || null // Use null default for location
       }));
   const userMessage = JSON.stringify({ transactions: sanitizedTransactions });
   const systemPrompt = transactionAnalysisPrompt;
   let messageContent = ""; let modelUsed = "";
   try {
     if (config.analysisProvider === 'gemini') {
       ...
       modelUsed = config.gemini.previewModel; const tools: Tool[] = [{ googleSearch: {} }];
       ...
       const result = await genAI.models.generateContent({ model: modelUsed, contents: contents, config: { tools: tools, responseMimeType: 'text/plain' } });
       console.log("Gemini Raw Result Object:", JSON.stringify(result, null, 2));
       const finishReason = result?.candidates?.[0]?.finishReason; const safetyRatings = result?.candidates?.[0]?.safetyRatings; const blockReason = result?.promptFeedback?.blockReason;
       ...
       if (finishReason && finishReason !== FinishReason.STOP && finishReason !== FinishReason.MAX_TOKENS) { console.error(`Gemini response candidate finished unexpectedly. Reason: ${finishReason}`); console.error(`Safety Ratings: ${JSON.stringify(safetyRatings)}`); }
       messageContent = result?.candidates?.[0]?.content?.parts?.map(part => part.text).join('') || "";
       if (!messageContent) { console.error("Gemini returned empty messageContent."); let errorMessage = "Gemini returned empty content"; if (blockReason) { errorMessage += ` (Prompt blocked: ${blockReason})`; } else if (finishReason && finishReason !== FinishReason.STOP && finishReason !== FinishReason.MAX_TOKENS) { errorMessage += ` (Generation stopped: ${finishReason})`; } else { errorMessage += " (Check logs)"; } throw new Error(errorMessage); }
     } else {
       ...
       const messages: ChatCompletionMessageParam[] = [{ role: "system", content: systemPrompt }, { role: "user", content: userMessage },];
       const completionParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsNonStreaming = { model: modelUsed, messages: messages, response_format: { type: "json_object" }, };
       const rawResponse = await openai.chat.completions.create(completionParams);
       messageContent = rawResponse.choices[0]?.message?.content || ""; if (!messageContent) { throw new Error("OpenAI returned empty content."); }
     }
     let jsonString = messageContent.trim(); jsonString = jsonString.replace(/^```(?:json)?\s*/, '').replace(/\s*```$/, '');
     const firstBrace = jsonString.indexOf("{"); const lastBrace = jsonString.lastIndexOf("}");
     if (firstBrace === -1 || lastBrace <= firstBrace) { console.error("Raw API Response:", messageContent); throw new Error(`Could not find JSON boundaries in ${config.analysisProvider} response.`); }
     jsonString = jsonString.substring(firstBrace, lastBrace + 1); let parsedJson: unknown;
     try { parsedJson = JSON.parse(jsonString); }
     catch (parseError) { console.error(`JSON parsing error (${config.analysisProvider}):`, parseError); console.error("Attempted to parse:", jsonString); throw new Error(`Failed to parse JSON from ${config.analysisProvider}. Content: "${jsonString.substring(0,100)}...". Error: ${parseError instanceof Error ? parseError.message : String(parseError)}`); }
     const validationResult = AnalysisResponseSchema.safeParse(parsedJson);
     if (!validationResult.success) { console.error(`LLM Response Validation Failed (${config.analysisProvider}):`, validationResult.error.flatten()); console.error("Parsed JSON that failed:", JSON.stringify(parsedJson, null, 2)); throw new Error(`LLM response validation failed. Issues: ${JSON.stringify(validationResult.error.flatten())}`); }
     const validatedData = validationResult.data; console.log(`analyzeTransactionsViaAPI: Parsed and validated response from ${config.analysisProvider}.`); return validatedData.transactions;
   } catch (error) {
     if (hasErrorDetails(error)) { console.error("Gemini Error Details:", JSON.stringify(error.errorDetails, null, 2)); } else if (error instanceof Error) { console.error(`Error during ${config.analysisProvider} API call/processing (${modelUsed}):`, error.message, error.stack); } else { console.error(`Unknown error during ${config.analysisProvider} API call (${modelUsed}):`, error); }
     throw error;
   }
}
...
export function processTransactionList(
  ...
  const processedList = transactions.map((tx: Transaction): Transaction => {
    ...
    if (tx.citations && typeof tx.citations === 'object') {
      Object.entries(tx.citations).forEach(([practice, citationList]: [string, unknown]) => {
          ...
          processedCitations[practice] = citationList
            .map((cit: unknown): Citation | null => {
              ...
              if (isPotentialCitation(cit)) {
                ...
                if (potentialCitation.url.length > 0 && (potentialCitation.url.startsWith('http') || potentialCitation.url.startsWith('/'))) {
  ...
  const updatedTransactions = processedList.map((t: Transaction): Transaction => {
