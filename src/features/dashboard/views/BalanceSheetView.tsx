      ...
      <div className="flex justify-between items-start mb-1 gap-2">
        ...
        <div className="text-right flex-shrink-0">
          <AnimatedCounter
            ...
            decimalPlaces={0}
            title={`Precise: ${preciseFormatImpact(
...
export function BalanceSheetView({
  ...
  const impactAnalysis: ImpactAnalysis | null = useTransactionStore(
  ...
  const findDominantPracticeSearchTerm = useCallback(
    (categoryName: string): string => {
        ...
        (tx.unethicalPractices || []).forEach((practice) => {
          ...
          if (txCategoryName === categoryName) {
            ...
            practiceContributions[practice] = {
              ...
              term:
                tx.practiceSearchTerms?.[practice] ||
                practiceContributions[practice]?.term,
            };
          }
        });
      });
      ...
      const fallbackMappings: Record<string, string> = {
        ...
        "Labor Exploitation": "labor rights",
      };
      return (
        (dominantPractice && fallbackMappings[dominantPractice]) || categoryName
      );
    },
    [transactions]
  );
    ...
    async (categoryName: string) => {
      ...
      try {
        ...
        const netDebt = Math.max(
          0,
          (categoryData?.totalNegativeImpact ?? 0) -
            (categoryData?.totalPositiveImpact ?? 0)
        );
  ...
  const triggerEveryOrgWidget = useCallback(
    (charity: EnrichedCharityResult, amount: number) => {
      ...
      if (!charityIdentifier) {
        updateInlineOffsetState(categoryName, {
          errorMessage: "Could not identify charity for donation widget.",
        });
        return;
      }
      ...
      if (window.everyDotOrgDonateButton) {
        try {
          ...
          updateInlineOffsetState(categoryName, {
            errorMessage: "Could not initialize donation widget.",
          });
        }
      } else {
        console.error("Every.org widget script not loaded.");
        updateInlineOffsetState(categoryName, {
          errorMessage: "Donation service is unavailable.",
        });
      }
    },
  ...
  useEffect(() => {
    processedData.categories.forEach((category) => {
      if (!categoryInlineUIState[category.name]) {
        ...
        if (
          currentAmountState === undefined ||
          Math.abs(currentAmountState - newDefaultAmount) > 0.5
        ) {
          if (
            categoryInlineUIState[category.name]?.recommendationStatus !==
            "loading"
          ) {
            updateInlineOffsetState(category.name, {
  ...
  const UnifiedCategoryCard: React.FC<{
    ...
    const showMobileInlineOffsetUI =
      isExpanded && negativeImpactForOffset > 0.005;
    ...
    const handleInlineWidgetTrigger = (e: React.MouseEvent) => {
      ...
      const amount =
        inlineOffsetState?.donationAmount ?? Math.round(netDebtForCategory);
      ...
      <div className="card flex flex-col">
        ...
        >
          <div className="p-3 space-y-2 border-t border-slate-200 dark:border-slate-700">
            {category.negativeDetails.concat(category.positiveDetails)
              .length === 0 &&
              isExpanded && (
            ...
            {showMobileInlineOffsetUI && inlineOffsetState && (
              <div className="pt-3 border-t border-slate-200 dark:border-slate-700 mt-3 space-y-3">
                <p className="text-xs text-[var(--muted-foreground)] italic text-center">
                  Offset {formatCurrency(negativeImpactForOffset)} from{" "}
                  {categoryName}:
                </p>
                  ...
                  <div className="border rounded-md p-3 bg-white dark:bg-gray-700/[.5] space-y-2 border-slate-200 dark:border-slate-600">
                    ...
                    <div className="pt-2 border-t border-slate-200 dark:border-slate-600">
                      <div className="flex items-center justify-between space-x-2">
                        <label
                          htmlFor={`donationAmount-${categoryName}-mobile`}
                          className="sr-only"
                        >
                          Amount:
                        </label>
                        <div className="flex items-center flex-grow">
                          ...
                          <input
                            id={`donationAmount-${categoryName}-mobile`}
                            type="number"
                            min="1"
          ...
          <h3 className="text-xl font-semibold text-center text-[var(--card-foreground)] opacity-80 rounded-md py-1 bg-black/10 dark:bg-white/15">
            ...
            <span className="text-lg text-[var(--success)] dark:text-emerald-400">
              ...
              <AnimatedCounter
                ...
                prefix="$"
                decimalPlaces={0}
                title={`Precise Total Positive: ${formatCurrency(
          ...
          <h3 className="text-xl font-semibold text-center text-[var(--card-foreground)] opacity-80 rounded-md py-1 bg-black/5 dark:bg-white/10">
            ...
            <span className="text-lg text-[var(--destructive)] dark:text-rose-400">
              ...
              <AnimatedCounter
                ...
                prefix="$"
                decimalPlaces={0}
                title={`Precise Total Negative: ${formatCurrency(
