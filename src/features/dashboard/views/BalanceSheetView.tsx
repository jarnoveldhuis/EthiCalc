...
export function BalanceSheetView({ transactions }: BalanceSheetViewProps) {
  ...
  const [expandedPositiveCategories, setExpandedPositiveCategories] = useState<
    Record<string, boolean>
  >({});
  const [expandedNegativeCategories, setExpandedNegativeCategories] = useState<
    Record<string, boolean>
  >({});
  ...
  const {
    detailedPositiveImpacts,
    detailedNegativeImpacts,
    positiveCategoryTotals,
    negativeCategoryTotals,
  } = useMemo(() => {
    ...
    transactions?.forEach((tx) => {
      ...
      (tx.ethicalPractices || []).forEach((practice) => {
        ...
        const category =
          tx.practiceCategories?.[practice] || defaultPositiveCategory;
      ...
      (tx.unethicalPractices || []).forEach((practice) => {
        // Ensure category name consistency here:
        const category =
          tx.practiceCategories?.[practice] || defaultNegativeCategory;
        const weight = tx.practiceWeights?.[practice] || 0;
    ...
    Object.values(posImpacts).forEach((arr) =>
      arr.sort((a, b) => b.impactAmount - a.impactAmount)
    );
    Object.values(negImpacts).forEach((arr) =>
      arr.sort((a, b) => b.impactAmount - a.impactAmount)
    );
  ...
  const togglePositiveCategory = (categoryName: string) =>
    setExpandedPositiveCategories((prev) => ({
      ...prev,
      [categoryName]: !prev[categoryName],
    }));
  const toggleNegativeCategory = (categoryName: string) =>
    setExpandedNegativeCategories((prev) => ({
      ...prev,
      [categoryName]: !prev[categoryName],
    }));
      ...
      return (
        <div className="card p-6 text-center">
          <p className="text-[var(--card-foreground)] opacity-70">
            No transaction data available to display balance sheet.
          </p>
        </div>
      );
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-center text-red-700 dark:text-red-400 border-b border-[var(--border-color)] pb-2">
            ...
            <span className="text-lg">
              ({formatCurrency(negativeImpactTotalFromStore)})
            </span>
          </h3>
          {negativeCategoryTotals.length === 0 && (
            <div className="card p-6 text-center">
              <p className="text-[var(--card-foreground)] opacity-70">
                No negative impact generated.
              </p>
            </div>
          )}
          {negativeCategoryTotals.map(
            ({ name: categoryName, amount: categoryTotalAmount }) => (
              <div key={`neg-cat-${categoryName}`} className="card">
                ...
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ")
                      toggleNegativeCategory(categoryName);
                  }}
                >
                  ...
                  <div className="flex items-center">
                    <span className="font-bold text-red-600 dark:text-red-400 mr-2 text-sm sm:text-base">
                    ...
                    {/* Chevron Icon */}
                    <svg
                      className={`w-4 h-4 text-gray-400 dark:text-gray-500 transition-transform duration-2000 ${
                ...
                {expandedNegativeCategories[categoryName] && (
                  <div className="p-4 border-t border-[var(--border-color)] space-y-3 max-h-96 overflow-y-auto">
                    {(detailedNegativeImpacts[categoryName] || []).map(
                      (detail, index) => {
                        ...
                        const citationUrl =
                          fullTransaction?.citations?.[detail.practice];

                        return (
                          <div
                            key={`neg-detail-${index}`}
                            className="border-b border-[var(--border-color)] pb-3 last:border-b-0 last:pb-0"
                          >
                            <div className="flex justify-between items-start text-sm mb-1">
                              <div>
                                ...
                                <span className="text-xs text-[var(--card-foreground)] opacity-70 ml-2">
                              ...
                              <span className="font-medium text-red-600 dark:text-red-400">
                                {formatCurrency(detail.impactAmount)}
                              </span>
                            </div>
                            {detail.information && (
                    ...
                    {(detailedNegativeImpacts[categoryName] || []).length ===
                      0 && (
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-center text-green-700 dark:text-green-400 border-b border-[var(--border-color)] pb-2">
            Positive Spending{" "}
            <span className="text-lg">
              ({formatCurrency(positiveImpactTotalFromStore)})
            </span>
          </h3>
          {positiveCategoryTotals.length === 0 && (
            <div className="card p-6 text-center">
              <p className="text-[var(--card-foreground)] opacity-70">
                No positive impact generated.
              </p>
            </div>
          )}
          {positiveCategoryTotals.map(
            ({ name: categoryName, amount: categoryTotalAmount }) => (
              <div key={`pos-cat-${categoryName}`} className="card">
                ...
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ")
                      togglePositiveCategory(categoryName);
                  }}
                >
                  <div className="flex items-center">
                    {/* Use consistent icons */}
                  ...
                  <div className="flex items-center">
                    <span className="font-bold text-green-600 dark:text-green-400 mr-3 text-sm sm:text-base">
                      {formatCurrency(categoryTotalAmount)}
                    </span>
                    {/* Chevron Icon */}
                    <svg
                ...
                {expandedPositiveCategories[categoryName] && (
                  <div className="p-4 border-t border-[var(--border-color)] space-y-3 max-h-96 overflow-y-auto">
                    {(detailedPositiveImpacts[categoryName] || []).map(
                      (detail, index) => {
                    ...
                    {(detailedPositiveImpacts[categoryName] || []).length ===
                      0 && (
                      <p className="text-xs text-center text-[var(--card-foreground)] opacity-70">
                        No specific transactions found for this category.
                      </p>
