...
export function useCreditState(
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const state: UserCreditState = {
            userId: user.uid,
            availableCredit:
              typeof data.availableCredit === "number"
            ...
            lastAppliedAmount:
              typeof data.lastAppliedAmount === "number"
                ...
                : 0,
            lastAppliedAt:
              data.lastAppliedAt instanceof Timestamp
                ? data.lastAppliedAt
                : Timestamp.now(),
          };
        ...
        const totalPositiveImpact = calculationService.calculatePositiveImpact(transactionsCache.current); // Use the service
  ...
  const calculateAvailableCredit = useCallback(
    (currentTransactions: Transaction[]): number => {
      // Calculate total positive impact from current transactions using the service
      const totalPositiveImpact =
        calculationService.calculatePositiveImpact(currentTransactions);

      // Subtract any applied credit from the credit state
      const appliedCredit = creditState?.appliedCredit || 0;
      console.log(
        "Calculating available credit: PositiveImpact=",
        totalPositiveImpact,
        "AppliedCredit=",
        appliedCredit
      );
  ...
  const applyCredit = useCallback(
    async (amount: number): Promise<boolean> => {
      ...
      try {
        ...
        // Calculate how much credit we can actually apply using the service
        const totalDebt = calculationService.calculateNegativeImpact(
          transactionsCache.current
        );
        const availableCredit = calculateAvailableCredit(
          transactionsCache.current
        ); // Uses the updated calculateAvailableCredit
        const creditToApply = Math.min(amount, totalDebt, availableCredit);
        console.log(
          `Applying ${creditToApply} credit (debt: ${totalDebt}, requested: ${amount}, available: ${availableCredit})`
        );
        ...
        console.log(
          `Credit applied successfully. Remaining: ${remainingCredit}`
        );
        return true;
      } catch (err) {
  ...
  const refreshCreditState = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (isMounted.current) {
        setError(
          `Failed to refresh credit state: ${
            err instanceof Error ? err.message : String(err)
          }`
        );
      }
